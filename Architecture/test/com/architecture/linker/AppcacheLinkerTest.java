...
public class AppcacheLinkerTest {
	...
	private List<BindingProperty> ensureBinding(final List<SelectionDescriptor> values, final String permutationName, final int propertyCount) {
	...
	private List<BindingProperty> ensureBinding(final List<SelectionDescriptor> descriptors, final String permutationName) {
	...
	private Permutation addPermutation(final ArrayList<PermutationArtifact> artifacts, final String permutationName, final List<BindingProperty> bindings,
	...
	private void addSoftPermutation(final Permutation permutation, final List<BindingProperty> bindings, final HashSet<String> files) {
	...
	@Test
	public void calculatePermutation() throws UnableToCompleteException {
		final AppcacheLinker linker = new AppcacheLinker();
		final ArtifactSet artifacts1 = new ArtifactSet();
	...
	@Test
	public void perPermutationLink() throws UnableToCompleteException {
		final AppcacheLinker linker = new AppcacheLinker();
		final ArtifactSet artifacts1 = new ArtifactSet();
		artifacts1.add(new StandardGeneratedResource(Generator.class, "myapp.devmode.js", new byte[0]));
		artifacts1.add(new StandardGeneratedResource(Generator.class, "file1.txt", new byte[0]));
	...
	@Test
	public void createPermutationMap() throws UnableToCompleteException, IOException {
		final AppcacheLinker linker = new AppcacheLinker();
	...
	private String toContents(final EmittedArtifact artifacts) throws UnableToCompleteException, IOException {
	...
	public void getArtifactsForCompilation() {
		...
		artifacts1.add(new StandardGeneratedResource(Generator.class, "path1", new byte[0]));
		artifacts1.add(new StandardGeneratedResource(Generator.class, "path2", new byte[0]));
		final StandardGeneratedResource resource = new StandardGeneratedResource(Generator.class, "path3", new byte[0]);
		...
		final LinkerContext linkerContext = mock(LinkerContext.class);
		when(linkerContext.getModuleName()).thenReturn("myapp");
		final Set<String> files = linker.getArtifactsForCompilation(linkerContext, artifacts1);
		assertEquals(files.size(), 2);
	...
	@Test
	public void parseFallbackResources() throws Exception {
		final AppcacheLinker linker = new AppcacheLinker();
