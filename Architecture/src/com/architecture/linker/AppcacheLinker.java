...
import com.architecture.server.appcache.BindingProperty;
import com.architecture.server.appcache.ManifestDescriptor;
import com.architecture.server.appcache.Permutation;
import com.architecture.server.appcache.PermutationsIO;
import com.architecture.server.appcache.SelectionDescriptor;
...
@Shardable
public final class AppcacheLinker extends AbstractLinker {
	...
	@Override
	public ArtifactSet link(final TreeLogger logger, final LinkerContext context, final ArtifactSet artifacts, final boolean onePermutation) throws UnableToCompleteException {
		if (onePermutation) {
	...
	private ArtifactSet perCompileLink(final TreeLogger logger, final LinkerContext context, final ArtifactSet artifacts) throws UnableToCompleteException {
		...
		for (final PermutationArtifact permutation : permutationArtifacts) {
			...
			final Map<String, String> fallbackFiles = parseFallbackResources(logger, getConfigurationValues(context, FALLBACK_FILES_CONFIGURATION_PROPERTY_NAME));
	...
	@Nonnull
	final Map<String, String> parseFallbackResources(@Nonnull final TreeLogger logger, @Nonnull final Set<String> values) throws UnableToCompleteException {
		...
		for (final String line : values) {
			...
			if (2 != elements.length) {
				final String message = FALLBACK_FILES_CONFIGURATION_PROPERTY_NAME + " property value '" + line + "' should have two url paths separated by whitespace";
	...
	final ArtifactSet perPermutationLink(final TreeLogger logger, final LinkerContext context, final ArtifactSet artifacts) throws UnableToCompleteException {
	...
	private boolean shouldAddToManifest(final String path) {
		return !(path.equals("compilation-mappings.txt") || path.endsWith(".devmode.js") || path.endsWith(".cache.js.gz"));
	}

	/**
	 * Write a manifest file for the given set of artifacts and return it as a string
	 * 
	 * @param staticResources
	 *            - the static resources of the app, such as index.html file
	 * @param fallbackResources
	 *            the fall back files to add to the manifest.
	 * @param cacheResources
	 *            the gwt output artifacts like cache.html files
	 * @return the manifest as a string
	 */
	final String writeManifest(final TreeLogger logger, @Nonnull final Set<String> staticResources, @Nonnull final Map<String, String> fallbackResources,
	...
	final EmittedArtifact createPermutationMap(final TreeLogger logger, final Collection<PermutationArtifact> artifacts) throws UnableToCompleteException {
		try {
	...
	final List<SelectionDescriptor> collectPermutationSelectors(final TreeLogger logger, final Collection<PermutationArtifact> artifacts) {
	 ...
	 */
	final Permutation calculatePermutation(final TreeLogger logger, final LinkerContext context, final ArtifactSet artifacts) throws UnableToCompleteException {
		...
		if (null != permutation) {
			logger.log(Type.DEBUG, "Calculated Permutation: " + permutation.getPermutationName() + " Selectors: " + permutation.getSelectors());
		}
		return permutation;
	}
}