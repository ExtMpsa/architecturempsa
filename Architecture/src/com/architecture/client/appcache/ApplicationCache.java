...
public abstract class ApplicationCache {
	...
	/**
	 * Cancels the application cache download process. The call is ignored if no download in progress.
	 */
	public abstract void abort();

	/**
	 * Request that the browser swap in the new application cache. This may fail if there is no update or no application cache.
	 * 
	...
	/**
	 * Request that the browser update the application cache. This may fail if the application is not cached.
	 * 
	...
	/**
	 * Attempt to remove the application from the cache. This is achieved by forcing the server to return a 404 or 410 when updating the manifest.
	 * 
	 * <p>
	 * Even if the request succeeded, there is no guarantee that the application will be removed from the cache. The server may not obey the directive or may not be reachable. The current
	 * implementation attempts to reload the manifest and pass cookies back to the server so that the server returns a "410 GONE" response. However some versions of Safari do not seem to pass the
	 * cookie and thus removeCache will not result in the application cache being removed.
	 * </p>
	 * 
