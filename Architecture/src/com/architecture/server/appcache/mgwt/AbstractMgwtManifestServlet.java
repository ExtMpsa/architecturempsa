...
import com.architecture.server.appcache.AbstractManifestServlet;
import com.architecture.server.appcache.BindingProperty;

public class AbstractMgwtManifestServlet extends AbstractManifestServlet {
	...
	protected boolean handleUnmatchedRequest(final HttpServletRequest request, final HttpServletResponse response, final String moduleName, final String baseUrl,
			final List<BindingProperty> computedBindings) throws ServletException {
		...
		if (isIPhoneWithoutCookie || isIPadWithoutCookie) {
			...
			if (null != nonRetinaPermutations && null != retinaPermutations) {
				...
				System.arraycopy(retinaPermutations, 0, permutations, nonRetinaPermutations.length, retinaPermutations.length);
