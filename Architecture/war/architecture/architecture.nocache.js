...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "architecture" because\n';
  ...
  $wnd.__gwt_activeModules['architecture'] = {
 ...
 * "/architecture.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['architecture'].superdevmode = true;

  var expectedSuffix = '/architecture.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + architecture + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:architecture';
  var devModeSessionKey = '__gwtDevModeSession:architecture';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/architecture/architecture.nocache.js';
  ...
  $wnd.__gwt_activeModules['architecture'].canRedirect = true;
  $wnd.__gwt_activeModules['architecture'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING architecture';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/architecture?user.agent=' + ua + '&_callback=' + callback;
